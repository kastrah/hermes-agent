...
class TestBackendSelection:
    ...
    def test_fallback_no_keys_defaults_to_firecrawl(self):
        """No keys, no config → 'reach'."""
        from tools.web_tools import _get_backend
        with patch("tools.web_tools._load_web_config", return_value={}):
            assert _get_backend() == "reach"

    def test_config_reach(self):
        """web.backend=reach in config → 'reach' without API keys."""
        from tools.web_tools import _get_backend
        with patch("tools.web_tools._load_web_config", return_value={"backend": "reach"}):
            assert _get_backend() == "reach"
...
class TestCheckWebApiKey:
    ...
    def test_no_keys_returns_true_for_no_api_backend(self):
        from tools.web_tools import check_web_api_key
        assert check_web_api_key() is True
