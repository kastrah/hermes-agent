...
- Reach: DuckDuckGo HTML + Jina Reader (search, extract without API keys)
...
def _get_backend() -> str:
    ...
    if configured in ("parallel", "firecrawl", "tavily", "exa", "reach"):
        return configured

    # Fallback for manual / legacy config — pick the highest-priority premium
    # backend that has credentials configured. If none are configured, fall
    # back to the built-in no-API backend so web access still works out of box.
    for backend, keys in [
...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    with premium APIs or the built-in no-API reach backend.
    ...
    try:
        ...
        if backend == "reach":
            result_json = no_api_web_search(query, limit)
...
async def web_extract_tool(
    ...
    can work with premium APIs or the built-in no-API reach backend.
    ...
    try:
        ...
        if not safe_urls:
            ...
            if backend == "reach":
                results = await no_api_web_extract(safe_urls)
...
def check_web_api_key() -> bool:
    """Check if Hermes has any usable web backend configured.

    Historically this meant "an API key exists". It now also returns True for
    the built-in no-API reach backend so the web toolset remains available
    without premium credentials.
    """
    return bool(
...
if __name__ == "__main__":
    ...
    if web_available:
        ...
        if backend == "exa":
            ...
            print("   Using no-API reach backend (DuckDuckGo HTML + Jina Reader)")
        else:
...
WEB_SEARCH_SCHEMA = {
    "name": "web_search",
    "description": "Search the web for information on any topic. Returns up to 5 relevant results with titles, URLs, and descriptions. With the built-in no-API reach backend, you can also target channels using prefixes like 'reddit:', 'github:', 'youtube:', 'bilibili:', or 'v2ex:'.",
    "parameters": {
        "type": "object",
        "properties": {
            "query": {
...
WEB_EXTRACT_SCHEMA = {
    "name": "web_extract",
    "description": "Extract content from web page URLs. Returns page content in markdown format. Also works with PDF URLs (arxiv papers, documents, etc.) — pass the PDF link directly and it converts to markdown text. Pages under 5000 chars return full markdown; larger pages are LLM-summarized and capped at ~5000 chars per page. Pages over 2M chars are refused. With the no-API reach backend, Hermes also has structured readers for supported URLs like Reddit, GitHub, YouTube, Bilibili, RSS/Atom, and V2EX. If a URL fails or times out, use the browser tool to access it instead.",
    "parameters": {
        "type": "object",
        "properties": {
            "urls": {
...
WEB_REACH_DOCTOR_SCHEMA = {
    "name": "web_reach_doctor",
    "description": "Show which built-in no-API web channels Hermes can use right now, including optional adapters like gh and yt-dlp.",
    "parameters": {
        "type": "object",
        "properties": {},
    },
...
registry.register(
    name="web_reach_doctor",
    toolset="web",
    schema=WEB_REACH_DOCTOR_SCHEMA,
    handler=lambda args, **kw: no_api_web_doctor(),
    check_fn=check_web_api_key,
    requires_env=["EXA_API_KEY", "PARALLEL_API_KEY", "FIRECRAWL_API_KEY", "TAVILY_API_KEY"],
    emoji="🩺",
)
