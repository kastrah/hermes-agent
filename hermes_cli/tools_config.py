...
TOOL_CATEGORIES = {
    ...
    "web": {
        ...
        "setup_note": "Hermes also includes a no-API option powered by DuckDuckGo HTML search and Jina Reader extraction.",
        "icon": "🔍",
        "providers": [
            {
                "name": "No-API Web Reach",
                "tag": "Free - DuckDuckGo HTML + Jina Reader",
                "web_backend": "reach",
                "env_vars": [],
            },
            {
...
def _toolset_has_keys(ts_key: str) -> bool:
    ...
    if ts_key == "web":
        config = load_config()
        if config.get("web", {}).get("backend") == "reach":
            return True
