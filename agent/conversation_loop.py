    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                _rate_remaining = rate_limit_remaining(agent.provider, agent.model)
                if _rate_remaining is not None and _rate_remaining > 0:
                    _rate_msg = (
                        f"{agent.provider}/{agent.model} rate limit active — "
                        f"resets in {_fmt_rate_remaining(_rate_remaining)}."
                    )
                    agent._vprint(
                        f"{agent.log_prefix}⏳ {_rate_msg} Trying fallback...",
                        force=True,
                    )
                    agent._emit_status(f"⏳ {_rate_msg}")
