    ...
    def create(self, **kwargs) -> Any:
        ...
        try:
            with self._client.responses.stream(**resp_kwargs) as stream:
                for _event in stream:
                    ...
                    if event_type == "response.output_text.delta" or "output_text.delta" in event_type:
                        delta_text = getattr(_event, "delta", "")
                        if delta_text:
            ...
            if (
                ...
                and stream_output_items
            ):
                setattr(final, "output", stream_output_items)
        ...
        if content is None:
            recovered = "".join(stream_text_deltas).strip()
            if recovered and not has_function_calls:
                logger.warning(
                    "Codex auxiliary stream completed with empty output payload; recovered response from streamed text deltas."
                )
                content = recovered
