...
def build_session_context_prompt(
    ...
    *,
    ...
    if context.handover_note:
        lines.append("")
        lines.append("**Handover from the previous session:**")
        lines.append(context.handover_note)
...
class SessionEntry:
    ...
    handover_note: Optional[str] = None  # high-signal summary from the prior session
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.handover_note:
            result["handover_note"] = self.handover_note
        return result
...
class SessionStore:
    ...
    def __init__(self, sessions_dir: Path, config: GatewayConfig,
        ...
        self._handover_notes: Dict[str, str] = {}
    ...
    def _set_handover_note_locked(self, session_key: str, note: str) -> None:
        note = (note or "").strip()
        if not note:
            return
        self._handover_notes[session_key] = note

    def _pop_handover_note_locked(self, session_key: str) -> Optional[str]:
        return self._handover_notes.pop(session_key, None)

    def set_handover_note(self, session_key: str, note: str) -> None:
        """Store a handover note for the next or current session.

        If the session already exists, attach the note to that entry immediately
        so late-arriving reset summaries still surface on the active session.
        Otherwise cache it for the next session creation.
        """
        note = (note or "").strip()
        if not note:
            return
        with self._lock:
            ...
            if entry:
                ...
                self._set_handover_note_locked(session_key, note)

    def pop_handover_note(self, session_key: str) -> Optional[str]:
        """Consume a pending handover note for a session key."""
        with self._lock:
    ...
    def get_or_create_session(
        ...
        with self._lock:
            ...
            entry = SessionEntry(
                ...
                handover_note=self._pop_handover_note_locked(session_key),
            )
    ...
    def switch_session(self, session_key: str, target_session_id: str) -> Optional[SessionEntry]:
        ...
        with self._lock:
            ...
            db_end_session_id = old_entry.session_id
            self._pop_handover_note_locked(session_key)
...
def build_session_context(
    ...
    if session_entry:
        ...
        context.handover_note = session_entry.handover_note
