...
from gateway.platforms.routing import RoutingFacts, decide_message_routing
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def _message_mentions_other_handle(self, message: Message) -> bool:
        """Return True when Telegram entities explicitly address another bot/user handle."""
        if not self._bot:
            return False
        ...
        for source_text, entities in (
            ...
            for entity in entities:
                ...
                if entity_type == "mention":
                    ...
                    if offset < 0 or length <= 0:
                        continue
                    mention = source_text[offset:offset + length].strip().lower()
                    if mention.startswith("@") and mention != expected:
                        return True
    ...
    def _message_contains_handle_token(self, message: Message) -> bool:
        """Detect plain-text @handles Telegram did not emit as self-mentions."""
        if not self._bot:
            return False
        bot_username = (getattr(self._bot, "username", None) or "").lstrip("@").lower()
        expected = f"@{bot_username}" if bot_username else None
        handle_pattern = re.compile(r"(?<![\w.])@[A-Za-z][A-Za-z0-9_]{2,31}\b")
    ...
    def _command_targets_this_bot(self, message: Message) -> bool:
        """Return False for Telegram slash commands explicitly addressed to another bot."""
        if not self._bot:
            return True
        bot_username = (getattr(self._bot, "username", None) or "").lstrip("@").lower()
        expected = f"@{bot_username}" if bot_username else None
        ...
        if "@" not in command:
            return True
        return command[command.find("@"):].strip().lower() == expected
    ...
    def _should_process_message(self, message: Message, *, is_command: bool = False) -> bool:
        ...
        is_allowed_chat = (not allowed) or (chat_id_str in allowed) or (self._telegram_guest_mode() and self._message_mentions_bot(message))

        exclusive_bot_mentions = self._telegram_exclusive_bot_mentions()

        facts = RoutingFacts(
            is_dm=False,
            allowed_chat=is_allowed_chat,
            ignored_thread=False,
            free_response=(
                chat_id_str in self._telegram_free_response_chats()
            ),
            require_mention=self._telegram_require_mention(),
            authored_by_bot=self._is_from_bot(message),
            mentions_self=self._message_mentions_bot(message),
            mentions_other=exclusive_bot_mentions and self._message_mentions_other_handle(message),
            contains_other_handle=exclusive_bot_mentions and self._message_contains_handle_token(message),
            replies_to_self=self._is_reply_to_bot(message),
            replies_to_other_bot=exclusive_bot_mentions and self._is_reply_to_other_bot(message),
            matches_wake_pattern=self._message_matches_mention_patterns(message),
            command_targets_self=(not is_command or self._command_targets_this_bot(message)),
        )
        return decide_message_routing(facts).should_process
